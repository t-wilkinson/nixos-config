#!/usr/bin/env -S\_/bin/sh\_-xc\_"source\_\$(eval\_echo\_\$AGS_VIRTUAL_ENV)/bin/activate&&exec\_python\_-E\_"\$0"\_"\$@""
import argparse
import re
import os
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to the Hyprland config file (default: ~/.config/hypr/hyprland.conf).",
    )
    parser.add_argument(
        "--key", required=True, help="Configuration key to modify or remove."
    )
    parser.add_argument(
        "--value", help="New value for the configuration key (optional).", default=None
    )
    parser.add_argument(
        "--reset",
        action="store_true",
        help="Remove the specified key from the config file.",
    )
