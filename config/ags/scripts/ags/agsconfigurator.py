#!/usr/bin/env -S\_/bin/sh\_-c\_"source\_\$(eval\_echo\_\$AGS_VIRTUAL_ENV)/bin/activate&&exec\_python\_-E\_"\$0"\_"\$@""
import argparse
import json
import sys
import ast
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Update or remove a key from a JSON configuration file"
    )
    parser.add_argument(
        "--key",
        required=True,
        help="The key to be updated or removed (e.g., 'search.enableFeatures.actions')",
    )
    parser.add_argument(
        "--file", required=True, help="The path to the target JSON file"
    )
    parser.add_argument(
        "--value", help="The new value to assign (e.g., 'true', '42', '[1, 2, 3]')"
    )
    parser.add_argument(
        "--reset",
        action="store_true",
        help="If set, the key will be removed from the JSON file",
    )
