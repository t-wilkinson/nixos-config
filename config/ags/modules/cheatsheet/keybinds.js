...
const HYPRLAND_KEYBIND_CONFIG_FILE = userOptions.cheatsheet.keybinds.configPath
  ? userOptions.cheatsheet.keybinds.configPath
...
const Keybind = (keybindData, type) => {
  // type: either "keys" or "actions"
  const Key = (key) =>
    Label({
      // Specific keys
      vpack: "center",
  ...
  return Box({
    ...
    children: [
      ...
      Box({
        ...
        children: [binds, childrenSections],
      }),
    ],
  });
};
...
export default () => {
  const numOfTabs = Math.ceil(
    keybindList.children.length / KEYBIND_SECTIONS_PER_PAGE,
  );
  const keybindPages = Array.from({ length: numOfTabs }, (_, i) => ({
    ...
    child: Box({
      ...
      children: keybindList.children
        .slice(
          KEYBIND_SECTIONS_PER_PAGE * i,
          0 + KEYBIND_SECTIONS_PER_PAGE * (i + 1),
        )
