  ...
  if (!translations[lang]) {
    try {
      ...
      if (Extra_logs || lang === "Default")
        console.warn(
          `Failed to load language file, language code: ${lang}:\n`,
          error,
        );
      return;
    }
...
function init() {
  try {
    ...
    if (Extra_logs)
      console.log(
        getString("Initialization complete!") || "Initialization complete!",
      );
...
function getString(key) {
  if (key && !translations[currentLanguage]?.[key] && Extra_logs)
    console.warn(
      `${translations[currentLanguage]["Not found"] || "Not found"}:::${key}`,
    );
  return (
