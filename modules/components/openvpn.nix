...
let
  ...
  vpnSetupScript = pkgs.writeShellScript "vpn-setup" ''
    #!/bin/sh
    set -e
    cp "${vpnConfig1}" /run/openvpn-config-1
    echo "auth-user-pass ${config.age.secrets.vpn-pia-credentials.path}" >> /run/openvpn-config-1

    cp "${vpnConfig2}" /run/openvpn-config-2
    echo "auth-user-pass ${config.age.secrets.vpn-pia-credentials.path}" >> /run/openvpn-config-2
    exit 0
