...
apply_vim() {
	# Check if scripts/templates/nvim/colors.vim exists
	if [ ! -f "scripts/templates/nvim/colors.vim" ]; then
		echo "Template file not found for Neovim. Skipping that."
		return
	fi
	# Copy template
	mkdir -p "$HOME"/.cache/ags/user/generated/nvim
	cp "scripts/templates/nvim/colors.vim" "$HOME"/.cache/ags/user/generated/nvim/colors.vim
	# Apply colors
	for i in "${!colorlist[@]}"; do
		sed -i "s/{{ ${colorlist[$i]} }}/${colorvalues[$i]#\#}/g" "$HOME"/.cache/ags/user/generated/nvim/colors.vim
	done
